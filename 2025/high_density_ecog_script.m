...
folderFiguresCell = {fullfile(folderDataBase, 'LL20')};
...
%% OUTPUT FIGURE
                    ...
                    figure(figout)
                    nexttile
                    imagesc([-0.51 0.51],[1,numPlotVec],meanSpikesReferencedGoodOnly(1:end-10,notNanIndsCombined)')
                    colormap(brewermap([],colormapChoice))
                    caxis(desiredPlotBoundsSubj)
                    hold on
                    for chanGoodInd = 1:length(notNanIndsCombined)
                    ...
                    xlabel('Time (s)')
                    ylabel('Channels')
                    title(['Pt. 5 High Density - Average Spikes, FDR'])
                    set(gca,'FontSize',10)
                    ...
                    figure(figout)
                    nexttile
                    imagesc([min(tPlot) max(tPlot)+0.012],[1,numPlotVec],meanSpikesReferencedGoodOnly(1:end-9,notNanIndsCombined)')
                    colormap(brewermap([],colormapChoice))
                    caxis(desiredPlotBoundsSubj)
                    hold on
                    for chanGoodIndSub = 1:length(notNanIndsCombined)
                    ...
                    xlabel('Time (s)')
                    ylabel('Channels')
                    title(['Pt. 5 Subsampled - Average Spikes, FDR'])
                    set(gca,'FontSize',10)
                    xticks([-0.50 -0.25 0 0.25 0.50])
